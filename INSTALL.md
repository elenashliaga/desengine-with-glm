...
- `tasks/` содержит versioned-конфиги задач и картинки-референсы;
...
- Обычный reset отдельной задачи тоже должен удалять её пользовательский каталог полностью, без повторного восстановления старых legacy-следов этой задачи.
