...
- Канонические описания задач и картинки-референсы остаются в `tasks/`.
...
- Пользовательский reset задачи удаляет её рабочее состояние из `user/` целиком и не даёт старым legacy-артефактам этой задачи восстановиться обратно.
