...
- **AND** типовое значение `OPENAI_MODEL` уже присутствует в шаблоне
...
- **THEN** он читает `OPENAI_MODEL` из `desengine.config.txt` или env процесса
- **AND** не использует `desengine.config.json` как fallback для имени модели
