...
Система SHALL брать имя модели только из `config.txt` или эквивалентных env vars процесса, а не из `env/config.json`.
...
#### Scenario: В env/config.json нет настройки модели
- **WHEN** приложение читает инфраструктурный `env/config.json`
...
- **THEN** система явно предупреждает, что канонический локальный конфиг лежит в `config.txt`
