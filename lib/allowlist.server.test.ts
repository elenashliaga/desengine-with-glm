...
describe("allowlist HTTP contracts", () => {
  ...
  it("считает базовый URL allowlist готовым, если HEAD не работает, но GET отвечает 200", async () => {
    ...
    const { checkAllowlistSystemReachability } = await import("./allowlist.server")
  ...
  it("не считает 404 штатным ответом базового URL allowlist", async () => {
    const { summarizeAllowlistSystemStatus } = await import("./allowlist.server")
  ...
  it("сохраняет семантику marker-check: 404 означает отказ", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response(null, { status: 404 })))

    const { checkAllowlistMarkerReachability } = await import("./allowlist.server")
